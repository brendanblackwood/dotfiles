function fish_prompt --description 'Write out the prompt'
    ...
    if set -q VIRTUAL_ENV
        set venv (set_color white) "(" (basename "$VIRTUAL_ENV") ")" (set_color normal) " "
    end
    # replace $HOME with "~"
    set realhome ~
    ...
    echo -n -s $venv (set_color $fish_color_user) "$USER" $normal @: (set_color $color_cwd) $tmp $normal (__fish_vcs_prompt) $normal $prompt_status $suffix " "
end